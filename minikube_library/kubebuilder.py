...
def do_kubebuilder(s, pw):
	...
	s.multisend("""kubebuilder init --domain k8s.io --license apache2 --owner 'The Kubernetes Authors'""",{'ecommended':'y'})
	s.multisend('kubebuilder create api --group mygroup --version v1beta1 --kind MyArbitraryKind',{'Create':'y'})
	s.send('make install')
	s.send('make run')
	s.pause_point('https://book.kubebuilder.io/basics/project_creation_and_structure.html')
